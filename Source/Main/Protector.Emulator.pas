...
type
  (* ѕримерна€ структура объекта, в котором хранитс€ информаци€ о серверах с 4exchange,
     а также вспомогательные структуры дл€ распаковки gzip/deflate и некоторые HTTP
     заголовки. *)
...
(* ѕоиск двух переменных эмул€тора - ServerListRawSize и ServerList. ¬ первой
   находитс€ количество байт, записанных в ServerList, во второй же переменной
   находитс€ массив TNetAddress. *)
...
(* Ёмул€тор устроен так, что список серверов заноситс€ в базу сервербраузера один раз,
   затем нужно подождать 90 секунд дл€ того, чтобы при следующем обновлении список
   занЄсс€ ещЄ раз. ƒанна€ функци€ снимает это ограничение, чтобы сервера заносились в
   базу каждый раз, как пользователь обновл€ет список серверов. *)
